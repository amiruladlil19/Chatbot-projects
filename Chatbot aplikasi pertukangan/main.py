...
def filter(order, df):
    ...
    if order[0] in perbaikan and not set(order).issubset(set(perbaikan)):
        for i in order:
            ...
            df_filtered = df[df['layananPengecatan'].str.contains(
                i, regex=False, case=False, na=False)]
    elif set(order).issubset(set(perbaikan)):
        for i in order:
            df_filtered = df[df['layananPerbaikan'].str.contains(
                i, regex=False, case=False, na=False)]
    elif set(order).issubset(set(pengecatan)):
        for i in order:
            df_filtered = df[df['layananPengecatan'].str.contains(
                i, regex=False, case=False, na=False)]
    elif order[0] in pengecatan and not set(order).issubset(set(pengecatan)):
        for i in order:
            df_filtered = df[df['layananPengecatan'].str.contains(
                i, regex=False, case=False, na=False)]
            df_filtered = df[df['layananPerbaikan'].str.contains(
